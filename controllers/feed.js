...
exports.getPosts = async (req, res, next) => {
  ...
  } catch (err) { // catch jest formatowany w blok .then()? (behind the scenes)
    if (!err.statusCode) {
      err.statusCode = 500;
    }
    next(err);
...
exports.createPost = async (req, res, next) => {
  ...
  const post = new Post({
    ...
    content: content,
    imageUrl: imageUrl,
    creator: req.userId // moongose skonwertuje to na userID
  });
  try {
    await post.save();
    ...
    await user.save();
    io.getIO().emit('posts', { // emit - all logged users | broadcast - full users
      action: 'create',
...
exports.updatePost = async (req, res, next) => {
  ...
  try {
    ...
    if (post.creator._id.toString() !== req.userId) { // spr czy usuwa to właściciel postu (autor)
